...
* 2017 Apr 2nd:  improved ACPI (FADT, GTDT, SPCR, MADT).
...
- Just SPCR, FADT, GTDT, BGRT, MADT (only describing CPU interfaces, not
  the GPU interrupt controller).

  Also, ACPI support is inherently a science project, as there is no
  standard way for doing ACPI on RPi3 (...custom interrupt controller).
  The only other ACPI implementations (MS-IoT and Minoca) are for 32-bit
  OSes and mutually incompatible. This will be a third mutually incompatible
  one ;-).
